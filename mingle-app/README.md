...
2. Sends the finalized transcript to `/api/translate/finalize` (or `/api/ios/v1.0.3/translate/finalize`)
...
- iOS versioned: `ios/v1.0.3` -> `/api/ios/v1.0.3/{existing-path}`
- Android versioned: `android/v1.0.3` -> `/api/android/v1.0.3/{existing-path}`
...
- Allowed values: `''`, `ios/v1.0.0`, `android/v1.0.0`, `ios/v1.0.3`, `android/v1.0.3`
- Example: `https://your-app/ko?apiNamespace=android/v1.0.3`
...
- Namespace examples:
  - iOS: `POST /api/ios/v1.0.3/client/version-policy`
  - Android: `POST /api/android/v1.0.3/client/version-policy`
...
- `NEXT_PUBLIC_WS_URL`
- `NEXT_PUBLIC_API_NAMESPACE` (required on iOS: `ios/v1.0.3`)
- On iOS, if `NEXT_PUBLIC_API_NAMESPACE` does not match `ios/v1.0.3`, the app shows an error instead of loading the WebView.
...
`pnpm rn:ios` enforces `NEXT_PUBLIC_API_NAMESPACE=ios/v1.0.3` validation before launch.
`pnpm rn:android` enforces `NEXT_PUBLIC_API_NAMESPACE=android/v1.0.3` validation before launch.
