...
2. Sends the finalized transcript to `/api/translate/finalize` (or `/api/ios/v1.0.5/translate/finalize`)
...
- iOS versioned: `ios/v1.0.5` -> `/api/ios/v1.0.5/{existing-path}`
...
- Allowed values: `''`, `ios/v1.0.0`, `android/v1.0.0`, `ios/v1.0.2`, `android/v1.0.2`, `ios/v1.0.3`, `android/v1.0.3`, `ios/v1.0.4`, `android/v1.0.4`, `ios/v1.0.5`, `android/v1.0.5`
...
- Namespace examples:
  - iOS: `POST /api/ios/v1.0.5/client/version-policy`
...
- `NEXT_PUBLIC_WS_URL`
- `NEXT_PUBLIC_API_NAMESPACE` (required on iOS: `ios/v1.0.5`)
- On iOS, if `NEXT_PUBLIC_API_NAMESPACE` does not match `ios/v1.0.5`, the app shows an error instead of loading the WebView.
...
`pnpm rn:ios` enforces `NEXT_PUBLIC_API_NAMESPACE=ios/v1.0.5` validation before launch.
