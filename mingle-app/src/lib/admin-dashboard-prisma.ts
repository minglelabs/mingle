...
/**
 * Dedicated client for the admin dashboard's read-only queries.
 *
 * Deliberately separate from `@/lib/prisma`: this reads
 * ADMIN_DASHBOARD_DATABASE_URL instead of DATABASE_URL, so pointing the
 * dashboard at production doesn't also point conversation/auth/etc. writes
