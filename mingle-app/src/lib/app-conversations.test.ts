...
describe("app-conversations", () => {
  ...
  it("preserves aggregate reads, membership and canonical message order during hydration", async () => {
    ...
    mockAppMessageFindMany.mockResolvedValue([{
      ...
      createdAt: time, metadata: {
        orderStartedAtMs: time.getTime() - 60000,
        translationTargetLanguages: ["ko", "ja"],
      }, contents: [{ contentType: "SOURCE", language: "en", text: "Hello" }],
    }]);
    ...
    expect(state?.utterances[0]).toMatchObject({
      serverCreatedAtMs: time.getTime() - 60000,
      createdAtMs: time.getTime(),
      serverMessageId: "db-message",
