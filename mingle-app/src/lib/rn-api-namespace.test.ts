...
describe('RN api namespace validation contract', () => {
  it('normalizes leading and trailing slashes', () => {
    expect(normalizeApiNamespace(' /ios/v1.1.0/ ')).toBe('ios/v1.1.0')
  })

  it('returns expected namespace by runtime os', () => {
    expect(resolveExpectedApiNamespace('ios')).toBe('ios/v1.1.0')
    expect(resolveExpectedApiNamespace('android')).toBe('android/v1.1.0')
  ...
  it('accepts only matching iOS namespace', () => {
    const result = validateRnApiNamespace({
      runtimeOs: 'ios',
      configuredApiNamespace: 'ios/v1.1.0',
    })

    expect(result.expectedApiNamespace).toBe('ios/v1.1.0')
    expect(result.validatedApiNamespace).toBe('ios/v1.1.0')
  })

  it('accepts only matching Android namespace', () => {
    const result = validateRnApiNamespace({
      runtimeOs: 'android',
      configuredApiNamespace: 'android/v1.1.0',
    })

    expect(result.expectedApiNamespace).toBe('android/v1.1.0')
    expect(result.validatedApiNamespace).toBe('android/v1.1.0')
  })
