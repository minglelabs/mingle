...
describe('RN api namespace validation contract', () => {
  it('normalizes leading and trailing slashes', () => {
    expect(normalizeApiNamespace(' /ios/v1.0.7/ ')).toBe('ios/v1.0.7')
  })

  it('returns expected namespace by runtime os', () => {
    expect(resolveExpectedApiNamespace('ios')).toBe('ios/v1.0.7')
    expect(resolveExpectedApiNamespace('android')).toBe('android/v1.0.7')
  ...
  it('accepts only matching iOS namespace', () => {
    const result = validateRnApiNamespace({
      runtimeOs: 'ios',
      configuredApiNamespace: 'ios/v1.0.7',
    })

    expect(result.expectedApiNamespace).toBe('ios/v1.0.7')
    expect(result.validatedApiNamespace).toBe('ios/v1.0.7')
  })

  it('accepts only matching Android namespace', () => {
    const result = validateRnApiNamespace({
      runtimeOs: 'android',
      configuredApiNamespace: 'android/v1.0.7',
    })

    expect(result.expectedApiNamespace).toBe('android/v1.0.7')
    expect(result.validatedApiNamespace).toBe('android/v1.0.7')
  })
