...
describe('api-contract namespace guard', () => {
  ...
  it('accepts only allowed env namespace values', async () => {
    process.env.NEXT_PUBLIC_API_NAMESPACE = 'ios/v1.0.7'
    const contract = await loadApiContractModule()
    expect(contract.clientApiNamespace).toBe('ios/v1.0.7')
  })

  it('accepts Android env namespace values', async () => {
    process.env.NEXT_PUBLIC_API_NAMESPACE = 'android/v1.0.7'
    const contract = await loadApiContractModule()
    expect(contract.clientApiNamespace).toBe('android/v1.0.7')
    expect(contract.buildClientApiPath('/translate/finalize')).toBe('/api/android/v1.0.7/translate/finalize')
  })
