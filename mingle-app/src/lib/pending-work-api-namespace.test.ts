...
describe('2.1.0 pending-work upgrade compatibility', () => {
  it.each(['android', 'ios'])('recovers older %s 2.x work without adopting another platform', platform => {
    const namespaces = compatiblePendingWorkNamespaces(`${platform}/v2.1.0`)
    expect(namespaces).toEqual([`${platform}/v2.0.0`, `${platform}/v2.0.1`, `${platform}/v2.0.2`, `${platform}/v2.0.3`, `${platform}/v2.0.4`, `${platform}/v2.1.0`])
