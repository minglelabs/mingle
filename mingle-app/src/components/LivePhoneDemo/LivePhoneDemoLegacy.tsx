...
import { useState, useRef, useEffect, useLayoutEffect, useImperativeHandle, forwardRef, useCallback, useMemo, useId, useSyncExternalStore, type ChangeEvent, type FormEvent } from 'react'
...
const LivePhoneDemo = forwardRef<LivePhoneDemoRef, LivePhoneDemoProps>(function LivePhoneDemo({
  ...
  useEffect(() => {
    const handlePopState = (event: PopStateEvent) => {
      ...
      if (isNativeIosHistoryGesture) {
        langSelectorIosHistorySettleRef.current = {
          open: nextStateOpen,
          expiresAt: nowMs + LANG_SELECTOR_IOS_HISTORY_SETTLE_WINDOW_MS,
        }
        return
      }
  ...
  useEffect(() => registerNativeBackHandler(() => {
    ...
    return false
  }, 10), [closeLanguageSelector, langSelectorOpen])
