...
1. **Original speech and translated outputs were rendered as separate stacked bubbles**
   ...
   Fix: Each utterance now uses one white message bubble that displays one language at a time. The circular language buttons are appended directly before the displayed text without a separate row and are rendered at half their previous size; selecting one switches the same bubble between the original text and the corresponding translation. The default display language comes from the signed-in user's profile primary language when that language is available, with the source language as the fallback. Only the original-language button carries the small white badge with a large black closing-quote icon. The message copy button now copies only the currently displayed language, and its confirmation toast uses a black background with white text for contrast. Existing copy-all and selected-language pronunciation actions remain available.
