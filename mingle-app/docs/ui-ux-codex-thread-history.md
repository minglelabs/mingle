...
- `2026-09-18-keyboard-composer-attachment-touch` | In an authenticated conversation's text-input mode, the plus button must open the attachment menu so a user can choose the existing `Hide keyboard` action and return to voice mode. The trigger intentionally prevented pointer-down's default behavior to keep the textarea and software keyboard stable, but iOS WKWebView can suppress the synthetic click that the previous implementation relied on. The trigger now opens its menu on pointer-up, while ignoring the follow-on pointer click when one is emitted; keyboard and assistive-technology click activation remains available. The control now also exposes its expanded state. Automated interaction-contract coverage verifies the pointer-up and click paths. Physical-device confirmation remains pending because no iOS or Android device was connected during this change.
