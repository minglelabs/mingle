...
- `NEXT_PUBLIC_API_NAMESPACE` (iOS: `ios/v2.0.1`, Android: `android/v2.0.1`)
...
`pnpm rn:ios` validates `NEXT_PUBLIC_API_NAMESPACE=ios/v1.1.3` before launch.
`pnpm rn:android` validates `NEXT_PUBLIC_API_NAMESPACE=android/v1.1.3` before launch.
For release-safe 1.1.3 builds, `NEXT_PUBLIC_SITE_URL` and `NEXT_PUBLIC_WS_URL` must not point to the legacy 1.0.11 production hosts.
...
- iOS: `/api/ios/v1.1.3/client/version-policy`
- Android: `/api/android/v1.1.3/client/version-policy`
