...
`pnpm rn:ios` validates `NEXT_PUBLIC_API_NAMESPACE=ios/v1.0.4` before launch.
`pnpm rn:android` validates `NEXT_PUBLIC_API_NAMESPACE=android/v1.0.4` before launch.

On startup, the RN app calls the version-policy API and applies `force_update | recommend_update | none`.

- iOS: `/api/ios/v1.0.4/client/version-policy`
- Android: `/api/android/v1.0.4/client/version-policy`
