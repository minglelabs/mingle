...
describe('namespaceSupportsPostingFeed', () => {
  it('accepts v2.2.0 and above on both platforms', () => {
    expect(namespaceSupportsPostingFeed('ios/v2.2.0')).toBe(true);
    expect(namespaceSupportsPostingFeed('android/v2.2.0')).toBe(true);
    expect(namespaceSupportsPostingFeed('ios/v2.2.3')).toBe(true);
    expect(namespaceSupportsPostingFeed('android/v3.0.0')).toBe(true);
    expect(namespaceSupportsPostingFeed('/ios/v2.2.0/')).toBe(true);
  });

  it('rejects versions below v2.2.0', () => {
    expect(namespaceSupportsPostingFeed('ios/v2.1.0')).toBe(false);
    expect(namespaceSupportsPostingFeed('android/v2.1.9')).toBe(false);
  ...
  it('rejects empty or malformed namespaces (conservative fallback)', () => {
    ...
    expect(namespaceSupportsPostingFeed('  ')).toBe(false);
    expect(namespaceSupportsPostingFeed('ios/2.2.0')).toBe(false);
    expect(namespaceSupportsPostingFeed('web/v2.2.0')).toBe(false);
...
describe('resolveInitialWebRoute', () => {
  it('lands on the feed for posting-capable builds', () => {
    expect(resolveInitialWebRoute('ios/v2.2.0')).toBe('feed');
    expect(resolveInitialWebRoute('android/v2.2.0')).toBe('feed');
  });

  it('stays on the conversation list for older or unknown namespaces', () => {
    expect(resolveInitialWebRoute('ios/v2.1.0')).toBe('conversations');
