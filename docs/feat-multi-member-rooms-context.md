...
현재 tunnel 구현은 각 프로세스가 별도 public endpoint를 사용하도록 되어 있으므로 web/STT/messaging에 같은 hostname을 재사용하면 안 됩니다. 사용자가 Cloudflare에 messaging hostname과 DNS를 등록한 뒤 shell/devbox 환경에 반영하고, named tunnel을 재시작한 다음 양쪽 모바일 Release 앱을 다시 빌드해야 합니다.
...
3. Cloudflare에 messaging named hostname을 등록하고 `DEVBOX_CLOUDFLARE_MESSAGING_HOSTNAME`을 설정합니다.
