...
## 2026-08-06 - Final Translation Visibility Latency Telemetry

- Surface: `mingle-app/src/components/LivePhoneDemo/use-realtime-stt.ts`, `mingle-app/src/components/LivePhoneDemo/use-realtime-stt-legacy.ts`, `mingle-app/src/server/api/handlers/v1/log-client-event-handler.ts`
- Issue: The existing `stt_turn_finalized` event measured the finalization request lifecycle, not the interval a user sees between the source bubble becoming final and its translation bubbles becoming final. Retries and aborted requests could also overwrite or leave a duration that did not represent the rendered translation state.
- User impact: Translation-latency analysis could report a zero or otherwise misleading value even while translations were visible to users, making regressions hard to identify.
- Resolution: The client now records the source bubble's final rendered frame in memory. Once final translation bubbles have rendered, it emits one `translation_visibility` event in `app_event_logs`, linked to the corresponding message. Its `clientMetadata` contains both `translationFirstVisibleMs` and `translationAllVisibleMs`, plus the rendered target languages. With one translation, the two timestamps naturally match. The event is emitted only after the final state is rendered (not while interim gray translation text is shown) and waits for the source message log to be persisted so the two events share the same message identity.
- Tests: `translation-visibility-telemetry.test.ts` covers final-state detection, source-language filtering, and the single-translation case. `log-client-event-handler.test.ts` covers event validation, message linking, and metadata persistence.
