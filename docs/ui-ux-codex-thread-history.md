...
## 2026-04-26 - STT Text Composer Flicker

- Surface: `mingle-app/src/components/LivePhoneDemo/LivePhoneDemo.tsx`
- Issue: In conversation rooms with long histories and active STT/TTS activity, keyboard-mode typing could make the chat surface flicker slightly and make the text cursor feel unstable because each keystroke updated parent React state and re-rendered the full live demo tree.
- User impact: Manual text entry felt unstable while live playback or long chat surfaces were present.
- Resolution: Moved the composer draft into a ref-backed uncontrolled textarea and kept React state limited to send-button eligibility and textarea height, reducing chat surface churn during typing.

## 2026-04-26 - Mobile Swipe Close Conflict
...
- Issue: iOS conversation rooms and the right-side menu had app-level left-to-right swipe handlers that could close the room or menu independently of OS back gestures.
- User impact: Horizontal touches inside a conversation or menu could feel risky and conflict with expected native navigation behavior.
- Resolution: Removed the app-level swipe-to-close handlers while preserving explicit controls, backdrop tap, browser/native history handling, and OS back behavior as supported dismissal paths.
