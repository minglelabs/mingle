...
- Resolution: Added a short-mobile-only media block for the affected slides. Slides 8, 11, 12, 14, 17, and 18 now use top-aligned compact layouts with reduced spacing and media typography; slide 12 wraps the prompt and constrains the reference image; slide 13 retains the domestic/global headings and first two representative categories per region while hiding the lower-priority rows only on short mobile screens; slide 18 uses smaller, width-safe ecosystem columns. Desktop and regular-height mobile rules remain unchanged.
- Verification: Checked all 20 slides at `375×568` for `ko`, `en`, `ja`, `cn-ZH`, and `cn-TW`; every run had no title clipping, external viewport overflow, or direct slide-content boundary overflow. Rechecked all five locales at `390×844`, and confirmed the PC layout at `1440×900` keeps the mobile class/media query inactive and all eight slide-thirteen data blocks visible. `git diff --check` passed.
