...
- `2026-04-26-stt-text-composer-flicker` | In mingle-app conversation rooms with long histories and active STT/TTS activity, keyboard-mode typing could make the chat surface flicker slightly and make the text cursor feel unstable. The composer textarea was controlled by parent React state, so every keystroke re-rendered the full LivePhoneDemo tree, including the long chat list, scroll derivation, Framer Motion layout surfaces, and native bottom-bar clearance sync. The composer draft now stays in a ref-backed uncontrolled textarea, while React state only tracks send-button eligibility and textarea height. This keeps typing local to the input except when the empty/non-empty state or actual height changes, reducing visible chat churn during active playback.
- `2026-04-26-mobile-swipe-close-conflict` | iOS conversation rooms and the right-side menu had app-level left-to-right swipe handlers that could close the room or menu independently of the OS back gesture. This overlapped with iOS edge-swipe back expectations and made horizontal touches inside a conversation feel risky, while Android already has a clear system back affordance. The web conversation runtime now removes those app-level swipe-to-close handlers and keeps explicit controls, backdrop tap, browser/native history handling, and OS back behavior as the supported dismissal paths. The legacy menu runtime received the same menu-panel cleanup so older namespace entries do not preserve a conflicting close gesture.
