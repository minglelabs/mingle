...
## 2026-09-01 — Diagnose edge-swipe list interaction and STT latest-preview rollback

- Surface: iOS conversation-room edge-swipe dismissal, the conversation list immediately underneath the room, and the list row preview while a live STT session continues in the background.
- Issue A: `SlideSurface.requestClose` keeps the room surface `open` while its 320ms exit animation is running. `handleCloseActiveConversation` only clears `activeConversation` and starts `history.back()` after that animation resolves. The list is therefore visually exposed during the last part of the swipe while the room surface still owns pointer/touch handling and native edge-swipe suppression. The list-level `pendingConversationHistoryBackRef` is also set only in the post-animation close callback, so it cannot protect or queue taps made during the exposed transition window. Keeping a live STT room mounted adds additional React work to the same commit, which can lengthen the apparent blocked-input interval on iOS.
- User impact A: After an edge-swipe-back, the list can blink into view and appear rendered, but a tap or scroll is ignored until the room exit/history transition settles. This is a transition ownership and main-thread scheduling problem, not a server loading spinner.
- Issue B: `LivePhoneDemo` emits a transient preview for every non-final STT update and then emits the locally finalized text. `ConversationList` correctly displays the transient preview first and updates the local summary on finalization, but a realtime push or fallback poll can start `performConversationListRefresh` during the same interval. `mergeConversationLists` replaces the current row with the incoming server summary without comparing `latestMessageAt`, a client message id, or a local finalization revision. If the GET observes the previous persisted message (or a read path briefly lags the write), it overwrites the just-finalized local preview. A later push/poll restores the final text, producing the observed `partial → previous message → final message` sequence.
- User impact B: Interim Korean jamo or partial phrases such as `ㅇ`, `ㄴ`, and an expanding sentence are expected provider updates while speech is live. The regression is the short rollback to the prior message after the turn has locally finalized; it is nondeterministic because it depends on STT, translation, persistence, push, and poll timing. The existing transcript timestamp-preservation fix protects room ordering, but the list-preview path has no equivalent stale-summary guard.
- Diagnostics needed to confirm in production-like runs: record the room id, preview kind (`interim` or `final`), client message id/utterance id, local preview timestamp, list-refresh trigger (`push` or `poll`), and incoming `latestMessageAt`/preview age. For the edge-swipe path, record the gesture start, `requestClose` start/end, `onClose`, `history.back`, `popstate`, and first list pointer event. Do not record transcript text in diagnostics.
- Status: Diagnosis only. No interaction or preview behavior was changed in this entry.
