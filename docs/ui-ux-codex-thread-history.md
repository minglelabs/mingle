...
- `2026-04-18-xr-mobile-responsive-layout` | The XR deck's mobile path previously reused the desktop 1600x900 canvas by forcing the mobile viewport to the desktop design width and scaling the whole page down. That preserved desktop composition but made Korean canvas text and side media read too small or slightly different from normal DOM text on phones. The deck now keeps the desktop path unchanged for wide viewports, switches mobile to real device-width rendering, stacks each existing slide's left copy above its right-side media, scales the mobile copy/media rules independently, and keeps every mobile page mapped one-to-one with the same 61 desktop slides. Touch paging was also retuned so swiping left/up moves to the previous slide and swiping right/down moves to the next slide.
- `2026-04-18-xr-mobile-stack-spacing` | The first responsive mobile pass used `space-between` for slides with side media, so the copy block hugged the upper edge while the visual block was pushed toward the bottom of the phone screen. The mobile side-media slide shell now uses a top-aligned flex layout with a controlled vertical gap, keeping the copy near the top with breathing room and placing the visual block directly below it instead of anchoring it to the bottom.
- `2026-04-18-xr-mobile-media-scale` | After the mobile stack spacing change, the lower media blocks read a little undersized, especially slides that show two screenshots side by side. The mobile media containers and two-image screenshot compositions are now scaled up modestly while preserving the top-aligned stack, so the visual proof points feel closer to the desktop emphasis without returning to bottom-anchored placement.
- `2026-04-18-xr-mobile-conversation-frame-regression` | Scaling the generic mobile two-screenshot composition accidentally reapplied absolute positioning to the back screenshot inside the page 26 conversation-stack phone frame. That made the image detach from the iPhone shell and appear crushed beside a large notch. The conversation-stack-specific mobile override now restores the nested back screenshot to static, full-width frame content while leaving the direct two-image compositions enlarged.
- `2026-04-18-xr-mobile-youtube-aspect-ratio` | The page 10 YouTube Shorts iframe was intended to stay at 9:16, but the first mobile rule used a width cap plus a separate `max-height`, so the browser could clamp only the height and visually squash the player. The mobile short-video rule now derives both width and height from the same 9:16 constraints and removes the independent height clamp, keeping the embedded video ratio fixed.
- `2026-04-18-xr-mobile-numbered-card-padding` | The mobile-only numbered tool cards on pages 19, 24, 27, and 28 lost the same left text inset used by nearby mobile cards because the mobile `slide-tool-item--numbered` override reset padding to an even `13px 14px`. That placed the heading almost directly against the orange left rail even though the desktop layout remained correct. The mobile numbered-card padding now restores the left inset while leaving the desktop card spacing unchanged.
- `2026-04-18-xr-mobile-strategy-arrows` | Pages 27 and 28 lost their viral-loop arrow explanations on mobile because the general mobile deck rule hid all `slide-step-transition` elements to avoid cramped desktop arrows. The strategy viral-loop slides now get a mobile-only vertical connector treatment: the three stage cards stay in normal top-to-bottom order with controlled spacing, and the existing arrow labels reappear as compact connector chips between the cards. Desktop arrow positioning is unchanged.
